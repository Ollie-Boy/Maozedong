...
1. **书库页（Library）**
   ...
   - 内置 **毛泽东诗词** 全文（**131 篇**），资源为 `Resources/BundledPoetryCorpus_part*.txt`；**# 标题 → 加粗日期行 → 正文**（含 `1919年3月12日` 等整行日期）；注释区在 `---` 后仅保留正文，**去掉每行「注释：」前缀**，无内容则不显示注释块；书库搜索为 **离线 TextField**（非系统 `searchable`）
   - **阅读主题**（浅色/深色/护眼）通过 UIKit 外观同步到 **导航栏、搜索栏** 等系统控件；状态栏随深色主题反色
   - 有阅读进度时列表显示 **已读** 标记
...
2. **阅读页（Reader）**
   - 导航栏、底部工具栏与正文同色；横向翻页时 **标题由 `ReaderView` 的 toolbar principal 显示**，避免空标题在新系统上闪白条；诗词/选集横向翻页 **不显示** 底部分页圆点
   - 正文优先使用 **仿宋**（`STFangsong` 等），缺省回退宋体/衬线体
