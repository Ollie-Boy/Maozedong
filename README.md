...
1. **书库页（Library）**
   - 按内容类型分组展示：**诗词**、**语录**、**文章**、**选集**（导入时按文件名与篇幅自动推测分类，也可长按或上下文菜单修改）
   - 内置 **毛泽东诗词** 全文（**131 篇**，编号 1–131，含注释；原稿缺第 36 篇故后续篇目顺延编号），资源为 `Resources/BundledPoetryCorpus_part*.txt`，启动时自动合并进书库（按标题去重；若曾装过旧版会再合并一次以修复资源路径问题）
   - 顶部 **搜索**：在标题与正文中全文检索
   - 可按分组筛选（工具栏「分组」）
   - 左滑删除单篇文档
   - 内置多篇示例（首次启动且书库为空时生成）
